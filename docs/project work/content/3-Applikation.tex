...
\begin{figure}[h]
    \centering
    \rotatebox{90}{
    	\includegraphics[width=0.85\textheight]
...
\end{figure}
\label{fig:good_convergence_chart}

In Abbildung \ref{fig:good_convergence} ist eine Konfiguration mit hohem $\alpha$ abgebildet. Dies führt dazu, dass 

\begin{enumerate}
\item der Algorithmus schnell konvergiert, wie man in Abbildung \ref{fig:good_convergence_chart} sehen kann und
\item der Algorithmus zu einer akzeptablen Lösung führt
\end{enumerate}

Abbildung \ref{fig:good_convergence_chart} zeigt die Anzahl verschiedener Lösungen, die gleichzeitig existieren.
